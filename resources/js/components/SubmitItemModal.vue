<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <!-- Modal Content Box -->
    <div class="relative w-full max-w-lg overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-300/80 transition-all duration-300">
      <!-- Glow decoration -->
      <div class="absolute -right-20 -top-20 h-40 w-40 rounded-full bg-blue-500/5 blur-3xl pointer-events-none"></div>
      <div class="absolute -left-20 -bottom-20 h-40 w-40 rounded-full bg-violet-500/5 blur-3xl pointer-events-none"></div>

      <!-- Header -->
      <div class="flex items-center justify-between border-b border-slate-100 p-6">
        <h3 class="text-xl font-bold tracking-tight text-slate-900 flex items-center gap-2">
          <svg class="h-5 w-5 text-blue-500 animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v3m0 0v3m0-3h3m-3 0H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          Submit Content for Review
        </h3>
        <button @click="closeModal" class="rounded-lg p-1.5 text-slate-400 hover:bg-slate-50 hover:text-slate-700 transition-colors">
      ...
      <form @submit.prevent="handleSubmit" class="p-6 space-y-5">
        <!-- Error Alert -->
        <div v-if="error" class="rounded-xl border border-red-200 bg-red-50 text-sm text-red-600 p-4 flex items-start gap-3">
        ...
        <!-- Input: Email -->
        <div>
          <label class="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-2">Author Email</label>
          <div class="relative">
        ...
        <div>
          <div class="flex items-center justify-between mb-2">
            <label class="block text-xs font-semibold uppercase tracking-wider text-slate-500">Content Body</label>
            <!-- Auto fill button -->
            <button 
              type="button" 
              @click="autoFill" 
            ...
            >
              <svg v-if="isGenerating" class="animate-spin h-3.5 w-3.5 text-blue-600 mr-0.5" fill="none" viewBox="0 0 24 24">
