    ...
    <div class="w-full max-w-2xl bg-white rounded-3xl border border-slate-200 shadow-2xl shadow-slate-350/80 overflow-hidden flex flex-col max-h-[90vh] transform transition-all duration-300 animate-in fade-in zoom-in-95">
      ...
      <div class="border-b border-slate-100 bg-slate-50/50 p-6 flex items-center justify-between shrink-0">
        <div class="flex items-center gap-3">
          <div 
            ...
            :class="isBan ? 'bg-red-500/10 border-red-500/20 text-red-600 animate-pulse' : 'bg-rose-50 border-rose-100 text-rose-500'"
          >
          ...
          <div>
            ...
            <p class="text-xs text-slate-500">
              {{ isBan ? 'Draft a formal permanent suspension notice due to repeated violations' : 'Draft a constructive rejection notice for the submitter' }}
            </p>
          </div>
        </div>
        ...
        <div class="space-y-3 bg-slate-50 p-4 rounded-2xl border border-slate-150 text-xs">
          ...
          <div class="grid grid-cols-[60px_1fr] items-center gap-2">
            ...
            <span class="font-semibold text-slate-700">
              {{ isBan ? 'Account Suspension & Permanent Ban Notice - ReviewQueue' : 'Submission Rejection Notice - ReviewQueue' }}
            </span>
          </div>
        </div>
        ...
        <div v-if="loadingDraft" class="flex flex-col items-center justify-center py-12 text-center space-y-4">
          ...
          <div class="space-y-1">
            ...
            <p class="text-xs text-slate-400 max-w-xs">
              {{ isBan ? 'Generating a formal account suspension letter detailing policy threshold breach.' : 'Generating a polite, context-aware notification using local Ollama model.' }}
            </p>
          </div>
        </div>
      ...
      <div class="border-t border-slate-100 bg-slate-50/50 p-6 flex items-center justify-end gap-3 shrink-0">
        ...
        >
          ...
          {{ isBan ? (sendEmail ? 'Send Suspension & Ban User' : 'Confirm Ban Only') : (sendEmail ? 'Send Notification & Reject' : 'Confirm Rejection Only') }}
        </button>
      </div>
    </div>
  </div>
